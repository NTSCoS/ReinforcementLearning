...
def choose_action(state, q_table):
    ...
    if (np.random.uniform() > EPSILON) or (
        (state_actions
         == 0).all()):  # act non-greedy or state-action have no value
    ...
    for episode in range(MAX_EPISODES):
        ...
        while not is_terminated:
            ...
            S_, R = get_env_feedback(
                S, A)  # take action & get next state and reward
            q_predict = q_table.loc[S, A]
            if S_ != 'terminal':
                q_target = R + GAMMA * q_table.iloc[
                    S_, :].max()  # next state is not terminal
            else:
